...
Tr_access Tr_allocLocal(Tr_level level, bool escape, Tr_exp init) {
    F_access f_access;
    if (init == NULL) {  // which will not be inited at the beginning of the function frag
        f_access = F_allocLocal(level->frame, escape, NULL);
    } else {
        f_access = F_allocLocal(level->frame, escape, unEx(init));
    }
...
void Tr_procEntryExit(Tr_level level, Tr_exp body, Tr_accessList formals) {
    T_exp body_exp = unEx(body);

    // add local variable initialization
    F_accessList locals = level->frame->locals;
    for (; locals; locals = locals->tail) {
        ...
        if (local->kind == inFrame) {  // init to mem
            ...
            body_exp = T_Eseq(init_to_mem, body_exp);
        } else {  // init to reg
            T_stm init_to_temp = T_Move(T_Temp(local->u.reg), local->init);
            body_exp = T_Eseq(init_to_temp, body_exp);
        }
    }

    // move body result to return-value register
    T_stm t_stm = T_Move(T_Temp(F_RV()), body_exp);
...
Tr_exp Tr_call(Temp_label func, Tr_expList args, Tr_level caller, Tr_level callee) {
    if (callee == Tr_outermost()) {
        return ex(F_externalCall(Temp_labelstring(func), toTExpList(args)));
    } else {
...
Tr_exp Tr_recordCreation(Tr_expList fields) {
    ...
    T_exp call_malloc = F_externalCall(String("malloc"),
                                       T_ExpList(T_Binop(T_mul, T_Const(WORDSIZE), T_Const(field_count)), NULL));
    T_stm ops = T_Move(T_Temp(record_ptr), call_malloc);
    ...
    return ex(T_Eseq(ops, T_Temp(record_ptr)));
}
...
Tr_exp Tr_strcmp(A_oper oper, Tr_exp cmp_result) {
    T_exp zero = T_Const(0);

    T_stm t_stm;
    switch (oper) {
        case A_eqOp:
            t_stm = T_Cjump(T_eq, unEx(cmp_result), zero, NULL, NULL);
            break;
        case A_neqOp:
            t_stm = T_Cjump(T_ne, unEx(cmp_result), zero, NULL, NULL);
            break;
        case A_ltOp:
            t_stm = T_Cjump(T_lt, unEx(cmp_result), zero, NULL, NULL);
            break;
        case A_leOp:
            t_stm = T_Cjump(T_le, unEx(cmp_result), zero, NULL, NULL);
            break;
        case A_gtOp:
            t_stm = T_Cjump(T_gt, unEx(cmp_result), zero, NULL, NULL);
            break;
        case A_geOp:
            t_stm = T_Cjump(T_ge, unEx(cmp_result), zero, NULL, NULL);
            break;
        default:
            printf("Translate: unexpected condition operator %d", oper);
            assert(0);
...
Tr_exp Tr_for(Tr_access loop_var_access, Tr_exp lo, Tr_exp hi, Tr_exp body_if, Tr_exp body_while, Temp_label done_label) {
    ...
    T_stm if_stm = T_Seq(T_Cjump(T_le, loop_var, limit, if_body_label, done_label),
                         T_Seq(T_Label(if_body_label), unNx(body_if)));
    ...
    T_stm body_stm = T_Seq(T_Label(while_body_label),
                           T_Seq(increment_stm,   // NOTE: increment loopvar should happen before loop
                                 T_Seq(unNx(body_while),  // NOTE: cause old value is already used in if_body
                                       T_Jump(T_Name(while_test_label), Temp_LabelList(while_test_label, NULL)))));
    T_stm while_stm = T_Seq(condition_stm, body_stm);

    // add break/done label
    T_stm done = T_Label(done_label);

    // concatenate together
    T_stm t_stm = T_Seq(init_stm, T_Seq(if_stm, T_Seq(while_stm, done)));
