...
%type <dec> vardec decs_nonempty_s
...
exp :lvalue {$$ = A_VarExp(EM_tokPos, $1);}
    ...
    |LPAREN RPAREN {$$ = A_SeqExp(EM_tokPos, NULL);}  // NOTE: can A_expList be NULL?
...
// lvalue  :ID %prec ARAYSUB {$$ = A_SimpleVar(EM_tokPos, S_Symbol($1));}
//         |lvalue DOT ID %prec ARAYSUB {$$ = A_FieldVar(EM_tokPos, $1, S_Symbol($3));}
//         |lvalue LBRACK exp RBRACK %prec ARAYSUB {$$ = A_SubscriptVar(EM_tokPos, $1, $3);}
//         |ID LBRACK exp RBRACK %prec ARAYSUB {$$ = A_SubscriptVar(EM_tokPos, A_SimpleVar(EM_tokPos, S_Symbol($1)), $3);}
...
decs    : {$$ = NULL;}  // NOTE: can A_decList be NULL?
        |decs_nonempty {$$ = $1;}
...
expseq  : {$$ = NULL;}  // NOTE: can A_expList be NULL?
